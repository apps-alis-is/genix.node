...
local function _exec_genix_cli(...)
...
local function _get_genix_cli_result(exitcode, stdout, stderr)
    if exitcode ~= 0 then
...
if _info.genixd == 'running' then
    ...
    local _exitcode, _stdout, _stderr = _exec_genix_cli('-datadir=data', 'getblockchaininfo')
    local _success, _output = _get_genix_cli_result(_exitcode, _stdout, _stderr)
    ...
    local _exitcode, _stdout, _stderr = _exec_genix_cli('-datadir=data', 'mnsync', 'status')
    local _success, _output = _get_genix_cli_result(_exitcode, _stdout, _stderr)
